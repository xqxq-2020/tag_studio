...
class DataExport(object):
    ...
    @staticmethod
    def create_dataset_info(name: str, desc: str, dataset_type: str, user: str, purview: bool):
        import json
        # 请求头设置
        headers = {
        ...
        data = {
            ...
            "user": "15e3087d-e59c-438d-8a99-971c81c02a66",
            "purview": "true"
        }

        url = "http://localhost:27010/dataset"
        # request调用开发中心的创建数据集接口
        response1 = requests.post(url, headers=headers, json=data)
        id_res = eval(response1.text)['did']
        # 导入数据集时的参数
        import_data = {
            "did": id_res, "path": "/home/zksc/PaddleX-develop/dataset/plane_voc_pre"
        }
        # request调用开发中心的导入数据集接口
        response2 = requests.put(url, headers=headers, json=import_data)
        res2 = eval(response2.text)["status"]
        if res2 == 1:
            data2 = {"user": "15e3087d-e59c-438d-8a99-971c81c02a66", "did": id_res}
            # request调用开发中心的查询数据集详情接口
            response = requests.get(url, headers=headers, json=data2)
            # 解码为 Python 对象
            decoded_data = json.loads(response.text)
            return decoded_data
        else:
            return eval(response2.text)
